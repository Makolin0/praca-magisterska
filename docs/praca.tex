...
Sprawdziłem w jakim stopniu zmiana zbioru danych w środku uczenia wpływa na proces optymalizacji podczas uczenia. Na rysunku~\ref{fig:loss_comparison} widać krzywe funkcji straty dla obu wariantów, kontrolny po lewej stronie i badawczy po prawej. Zestawiając je ze sobą można zaobserwować można oczywistą różnicę, dwa gwałtowne przyrosty błędu w miejscach zmiany danych uczących. Taki wynik był jak najbardziej spodziewany, gdyż jednocześnie silnik próbuje przewidzieć trudniejsze zagrania oraz sam fakt zmiany danych uczących do których dopasowuje się silnik się zmienia, natomiast skok jest wyższy niż tego bym się spodziewał. Skoki błędu są niemalże identyczne do początkowego błędu przy starcie uczenia. 
...
Analiza odchyleń tych samych rysunków nie przekonuje również o prawdziwości hipotezy \textbf{H3}, która mówiła o nie przekraczaniu przez silnik Elo którym był uczony. Przez większość uczenia Elo spadało bądź wracało do stanu początkowego po zmianie koszyka danych uczących. Nie jestem przekonany czy można obalić hipotezę wygenerowanymi danymi, gdyż istnieje potencjalny problem zbyt krótkiej ilości epok dla każdego przedziału, przez co pogorszenie rangi w czasie mogło być tak naprawdę fragmentem tymczasowego regresu, tym bardziej że proces pod koniec nauki zaczynał wyrównywać Elo, a nawet z powrotem rosnąć.

Analizując Rysunek, \ref{fig:human_like_comparison} najwyższy pułap dopasowania w grupie kontrolnej (ponad 15\% względem zbioru 0-1800) znacząco przewyższa początkowe wskaźniki modelu badawczego z pierwszego etapu, co wskazuje, że samo zjawisko \textit{curriculum learning} nie ułatwia nauki naśladowania najsłabszych graczy i potencjalnie odrzuca hipotezę \textbf{H2}. W tych wynikach najbardziej zaskoczyło mnie to, że kontrolny silnik który przewidywał ruchy człowieka z szerszego przedziału umiejętności, lepiej sobie radził niż każdy silnik badawczy mający mniejszy zakres danych testowych, jak i również zasób wiedzy lepiej dopasowany do danych testowych.s
...
Na podstawie uzyskanych wyników oraz zaobserwowanych zjawisk, można sformułować kilka potencjalnych kierunków dla przyszłych eksperymentów z architekturą NNUE oraz metodą \textit{curriculum learning}:
